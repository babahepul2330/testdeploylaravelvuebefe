...
const rows = computed(() => [
  ...
  {
    title: "Action",
    routeEdit: { route: ROUTES.admin.product.edit, params: ["id"] },
...
const breadcrumbs = ref([
  ...
  { title: "Product", route: ROUTES.admin.product.index, current: true },
]);
...
function handleSearch() {
  router.push({
    path: ROUTES.admin.product.index,
    query: form
  })
}
...
<template>
  <div>
    ...
    <div class="flex flex-col lg:flex-row w-full mt-5 mb-8 gap-4">
      <h1 class="flex-1 text-2xl font-bold">List Product</h1>
      ...
      <div class="inline-flex items-center justify-between gap-5">
        ...
        <RouterLink :to="ROUTES.admin.product.create" class="flex-none btn btn-frame-outline btn-success btn-sm">
    ...
    <TableRender :pagination="data?.products" :rows="rows" :isLoading="isLoading" />
  </div>
</template>
